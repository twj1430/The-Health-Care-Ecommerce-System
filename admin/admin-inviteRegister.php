...
if (isset($_GET['invID'])) {
    $invID = $_GET['invID'];
    $_SESSION['invite_id'] = $invID;
    $invSQL = "SELECT * FROM admin WHERE admin_unique_id = '$invID'";
    $invQuery = mysqli_query($conn, $invSQL) or die($conn->error . __LINE__);
    if (mysqli_num_rows($invQuery) > 0) {
        while ($row = $invQuery->fetch_assoc()) {
            $admin_username_inv = $row['admin_username'];
...
if (isset($_POST['register'])) {
    ...
    $admin_upline_id = $_POST['admin_upline_id'];
    ...
    $identity_card_front_file = upload_Image('images/', $identity_card_front, $adminID);
    $identity_card_back = $_FILES['identity_card_back'];
    $identity_card_back_file = upload_Image('images/',  $identity_card_back, $adminID);
    $identity_card_holding = $_FILES['identity_card_holding'];
    $identity_card_holding_file = upload_Image('images/', $identity_card_holding, $adminID);
    ...
    if ($admin_password != $admin_confirm_password) {
        echo "<script>alert('Password Wrong'); window.location.assign('admin-inviteRegister.php?invID=" . $_SESSION['invite_id'] . "');</script>";
    } else {

        $CheckSQL = "SELECT * FROM admin WHERE admin_email = '$admin_email'";
        $query = mysqli_query($conn, $CheckSQL);
        if (mysqli_num_rows($query) > 0) {
            echo "<script>alert('Email has been used! Please try other emaill!'); window.location.assign('admin-inviteRegister.php?invID=" . $_SESSION['invite_id'] . "');</script>";
        } else {
            ...
            admin_ic_backImage, admin_ic_selfImage, admin_address, admin_post_code, admin_city, admin_country, admin_profile_image, admin_password, admin_create_time) VALUES 
            ('', '$adminID', '$admin_upline_id','Admin','$admin_username', '$admin_contact', '$admin_email', '$admin_fullname', '$admin_gender', '$admin_whatsapp', '$admin_wechat', '$admin_identity', '$identity_card_front_file',
            '$identity_card_back_file', '$identity_card_holding_file', '$admin_address', '$admin_post', '$admin_city', '$admin_country', 'avatar-1.png', '$admin_password_hash', NOW())";
...
<body>
    ...
    <div class="container">
        <div class="row">
            ...
            <div class="col-lg-9" align="center" style="margin: 0 0 20px 190px;">
                <div class="text-center">
                    <h3>Register Now!&nbsp;&nbsp;<img src="../assets/images/<?php if (isset($_GET['invID'])) {
                                                                                echo $admin_profile_image;
                                                                            } ?>" alt="avatar" width="50" style="margin-bottom: 25px;border-radius: 50px;"><span style="margin-left: 10px;color: green;"><?php if (isset($_GET['invID'])) {
                                                                                                                                                                                                                echo $admin_username_inv;
                                                                                                                                                                                                            } ?></span>&nbsp;Invite You</h3>
                </div>
                <hr>
            </div>

            <form action="admin-inviteRegister.php" method="POST" enctype="multipart/form-data">
                <input type="hidden" name="admin_upline_id" id="admin_upline_id" value="<?php if (isset($_GET['invID'])) {
                                                                                            echo $_GET['invID'];
                                                                                        } else { ?> - <?php } ?>">
                <div class="row">
                    ...
                    <div class="col-lg-9 offset-lg-2" style="margin-bottom:20px">
                        <div class="form-check form-check-inline">
                            <input class="form-check-input" type="radio" name="admin_gender" id="inlineRadio1" value="Male">
                            <label class="form-check-label" for="inlineRadio1">Male</label>
                        </div>
                        <div class="form-check form-check-inline">
                            <input class="form-check-input" type="radio" name="admin_gender" id="inlineRadio2" value="Female">
                    ...
                    <div class="col-lg-9 offset-lg-2" style="margin-bottom:20px">
                        <div class="row">
                            <div class="col-lg-6">
                                ...
                                <input type="text" name="admin_whatsapp" id="admin_whatsapp" class="form-control" style="border: none;background-color: #F1F3F5 ;padding:20px;" placeholder="WhatsApp" required />
                            </div>
                            <div class="col-lg-6">
                                <label>WeChat</label>
